...
## Known Issues (from PROJECT_MAP.md)

Watch for these existing bugs — don't let them spread or worsen:

| Issue | Location |
|-------|----------|
| `conversionRate` always returns 100% (divides by itself) | `conversion-service.ts` |
| Redirects to `/api/login` instead of `/login` | `auth-utils.ts` (client) |
| Search input is non-functional | `conversation-list.tsx` |
| WebSocket server has no authentication | `websocket-server.ts` |
| RBAC middleware returns hardcoded `operator` role in production | RBAC middleware |
| Audit log is in-memory only (lost on restart) | Audit log service |
| Email provider only logs to console (no real sending) | Email service |
| `isMessageStillValid()` always returns true; `markMessageAsSent/Failed` only log | `message-send.worker.ts` |
