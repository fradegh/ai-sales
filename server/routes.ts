...
export async function registerRoutes(
  ...
  app.get("/api/price-settings", requireAuth, async (req: Request, res: Response) => {
    try {
      const user = await getUserByIdOrOidcId(req.userId!);
      if (!user?.tenantId) {
        return res.status(403).json({ error: "User not associated with a tenant" });
  ...
  app.put("/api/price-settings", requireAuth, requirePermission("MANAGE_TENANT_SETTINGS"), async (req: Request, res: Response) => {
    try {
      const user = await getUserByIdOrOidcId(req.userId!);
      if (!user?.tenantId) {
        return res.status(403).json({ error: "User not associated with a tenant" });
  ...
  // POST /api/conversations/:id/price-lookup - manual price lookup (uses OEM from latest case)
  app.post("/api/conversations/:id/price-lookup", requireAuth, requirePermission("MANAGE_CONVERSATIONS"), async (req: Request, res: Response) => {
    try {
      const user = await getUserByIdOrOidcId(req.userId!);
      if (!user?.tenantId) {
        return res.status(403).json({ error: "User not associated with a tenant" });
  ...
  app.get("/api/conversations/:id/price-history", requireAuth, requirePermission("MANAGE_CONVERSATIONS"), async (req: Request, res: Response) => {
    try {
      const user = await getUserByIdOrOidcId(req.userId!);
      if (!user?.tenantId) {
        return res.status(403).json({ error: "User not associated with a tenant" });
