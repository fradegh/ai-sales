...
export class MemStorage implements IStorage {
  ...
  async upsertLearningQueueItem(item: InsertLearningQueueItem): Promise<LearningQueueItem> {
    ...
    if (existing) {
      ...
      const newReasons = [...new Set([...(existing.reasons ?? []), ...(item.reasons ?? [])])];
  ...
  async getRagChunksWithoutEmbedding(tenantId: string, limit?: number): Promise<{ id: string; chunkText: string }[]> {
    return [];
  }

  async getRagChunksWithStaleHash(tenantId: string, limit?: number): Promise<{ id: string; chunkText: string; storedHash: string | null; currentHash: string }[]> {
    return [];
  }
  ...
  // Vehicle Lookup Cache (stubs for MemStorage)
  ...
  // Vehicle Lookup Cases (stubs for MemStorage)
