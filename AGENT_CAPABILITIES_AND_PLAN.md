...
### 2.3а VehicleLookupWorker — поиск данных по VIN/FRAME

- **Параллельный запрос** — Podzamenu и PartsAPI вызываются одновременно через `Promise.all` (максимальная задержка ~75 с вместо ~285 с последовательно).
- **PartsAPI поддерживает FRAME** — номер кузова (например `NT32-517663`) теперь передаётся в PartsAPI так же, как VIN.
- **Fallback из vehicleMeta Podzamenu** — если PartsAPI не вернул make/model/year (типично для FRAME), значения берутся из `vehicleMeta` ответа Podzamenu.
- **Парсинг gearboxType из modifikaciya** — строка вроде `"5FM/T"` однозначно определяет тип КПП (`MT`/`CVT`/`AT`).

### 2.3б PriceLookupWorker — поиск и оценка цен
...
- **Идентификация модели КПП** — `isValidTransmissionModel` теперь отклоняет строки-типы (`CVT`, `AT`, `MT`, `АКПП` и др.), чтобы GPT всегда определял реальное название модели (например `JF016E`, `W5MBB`).
- **Метка типа КПП** — `null`/неизвестный тип → нейтральная метка `КПП` в запросах и ответах (ранее ошибочно использовалась `АКПП`). `AT` → `АКПП`, `MT` → `МКПП`, `CVT` → `вариатор`.
